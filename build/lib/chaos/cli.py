...
1. **Subsystem imports are lazy.** ``chaos --help``, ``init-db``,
...
LOG = logging.getLogger("chaos.cli")
...
#: Repository root: <root>/src/chaos/cli.py -> parents[2]
...
def build_settings(args: argparse.Namespace) -> Any:
    ...
    exactly the same ``CHAOS_*`` environment as the API service.
    """
...
def export_setting_overrides(args: argparse.Namespace) -> None:
    ...
    """
    if getattr(args, "database_url", None):
        os.environ["CHAOS_DATABASE_URL"] = args.database_url
    if getattr(args, "data_dir", None):
        os.environ["CHAOS_DATA_DIR"] = str(args.data_dir)
...
def _database_hint(url: str) -> str:
    ...
    return (
        "Check CHAOS_DATABASE_URL and that the target directory is writable. "
...
def cmd_load_registry(args: argparse.Namespace) -> int:
    ...
    load_package = resolve_subsystem(
        ...
        hint=(
            "The registry loader lives in src/chaos/registry/loader.py. "
            "Run 'chaos validate' first to confirm the design package itself is intact."
        ),
    )
    ...
    with open_session(settings, create=True) as session:
        try:
            ...
            raise CommandError(
                ...
                hint="Run 'chaos validate' to check the design package against its schemas.",
            ) from exc
...
def cmd_retention(args: argparse.Namespace) -> int:
    ...
    apply_retention = resolve_subsystem(
        "chaos.ingest.retention",
        ("apply_retention",),
        first_param="session",
        subsystem="Retention",
        hint="Retention lives in src/chaos/ingest/retention.py.",
    )
    ...
    print(
        f"  raw retention: {settings.historian_raw_retention_days} days (CHAOS_HISTORIAN_RAW_RETENTION_DAYS)"
    )
...
def cmd_backup(args: argparse.Namespace) -> int:
    ...
    restore_text = (
        ...
        "Restore\n"
        "  1. chaos init-db\n"
        "  2. chaos load-all              # rebuild from the design package, or\n"
        "  2b. restore tables/*.json with your own loader if the package has since changed\n"
        "  3. chaos status                # confirm counts match manifest.json\n\n"
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not counts:
        print("  (no platform tables present -- run 'chaos init-db')")
    ...
    if payload["missing_tables"]:
        print(f"\n{len(payload['missing_tables'])} declared table(s) missing -- run 'chaos init-db'")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            "All settings are overridable through CHAOS_* environment variables."
        ),
    ...
    parser.add_argument(
        "--database-url",
        help="Override CHAOS_DATABASE_URL for this invocation.",
    )
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    parser.add_argument(
        "--log-level",
        default=os.environ.get("CHAOS_LOG_LEVEL", "INFO"),
    ...
    serve = subparsers.add_parser(
        ...
        description=(
            ...
            "engine, EMS) start according to node role and the CHAOS_* settings."
        ),
    )
    serve.add_argument("--host", help="Bind address (default: CHAOS_API_HOST).")
    serve.add_argument("--port", type=int, help="Bind port (default: CHAOS_API_PORT).")
    serve.add_argument(
        "--reload",
        action="store_true",
    ...
    retention = subparsers.add_parser(
        ...
        description=(
            ...
            "CHAOS_HISTORIAN_RAW_RETENTION_DAYS; alarm and command audit records are kept "
