...
interlocks without editing this module::
    ...
    from chaos.commands.interlocks import global_registry
...
from chaos.models.commands import TERMINAL_COMMAND_STATES, Command
...
def physical_control_disabled(
    ...
    if not enabled and not request.dry_run:
        return InterlockResult.deny(
            PHYSICAL_CONTROL_DISABLED,
            "Physical control is disabled platform-wide (CHAOS_ALLOW_PHYSICAL_CONTROL is false)",
...
def global_registry() -> InterlockRegistry:
    ...
    import time; :class:`~chaos.commands.manager.CommandManager` picks
